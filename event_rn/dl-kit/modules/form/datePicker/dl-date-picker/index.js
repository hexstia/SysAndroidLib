...
import { BackHandler, PixelRatio, Platform, StyleSheet, Text, TouchableOpacity, View } from 'react-native';
...
export function showDatePicker(pickerSetting) {
  const datePicker = (
    <View style={{ left: 0, right: 0, top: 0, bottom: 0, height: 250, backgroundColor: '#FFF' }}>
...
class DLDatePick extends PureComponent {
  ...
  constructor(props) {
    ...
    if (this.props.timeMode == 'time' || this.props.timeMode == 'timePeriod') {
      this._selectedTime = reasonableTime;
    } else {
      this._selectedDate = reasonableTime.substr(0, 10);
      this._selectedTime = reasonableTime.substr(11, 5);
      this._yearAndMonth = reasonableTime.substr(0, 7);
    }
  }
